...
public class ECommCardDataProxyBuilder implements IPortalProxyBuilder
{
	...
	public Object createSharedWebServicesPortalProxy() throws Exception
	{
		...
		try
		{
			IConfiguration conf = new ECommCardDataConfigurationFactory(requestingSystem).createWebServicesEndpointforSS();

			createdProxy = new PersistService(null, conf.getServiceNameforSS()).getPersistService();
			...
			log.info(sMethod + "::Property value: " + conf.getWebservicesEndpointforSS());
			
			
			Map<String,Object> requestContext = ((BindingProvider)createdProxy).getRequestContext();
			requestContext.put("javax.xml.ws.service.endpoint.address", conf.getWebservicesEndpointforSS());
