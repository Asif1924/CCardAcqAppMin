...
BRB.IdentityVerificationController = function(activationItems, argTranslator, argMessageDialog) {
	...
	function scoreIdentityExamSuccess( argResponse ){
		...
		getScoringResult();			
		if (identityExamModel.isExaminationPassed()) {
			BRB.Log(sMethod + '::Examination is Finished');			
			invokeAdjudication(adjudicationSuccess, adjudicationFailed);
		}
		else if (identityExamModel.isExaminationFailed()) {
			invokeAdjudication(adjudicationSuccess, adjudicationFailed);
		}
		else if (identityExamModel.isFourthQuestionPresentInResponse()) {
	...
	//---------------------------------------------------------------------------------------
	function invokeAdjudication(argSuccessCB, argFailureCB ){
		var sMethod = "invokeAdjudication(argSuccessCB, argFailureCB )";
		BRB.Log(sMethod);
		new BRB.LoadingIndicatorController().showIdentityVerificationLoading();
		
		syncUserData();
		
		if (new BRB.CreditCardApplicationDataValidator(activationItems).isDataModelsValid()) {
			connectivityController.accountApplication(activationItems, argSuccessCB, argFailureCB);	
		} 
		else {
			showFaildPandingScreen();
	...
	function adjudicationSuccess(argResponse){
        ...
        if (argResponse.data && argResponse.data.appStatus && argResponse.data.appStatus == "APPROVED"){
        ...
        if (argResponse.error == true) {
    		BRB.Log(sMethod + '::Response Error::' + argResponse.msg);
//    		showFaildPandingScreen();
    		showTUFaildPandingScreen();
        }
        if (argResponse.data && argResponse.data.appStatus && argResponse.data.appStatus == "PENDING"){
        	BRB.Log(sMethod + '::Response PENDING::' + argResponse.msg);       	
        	showFaildPandingScreen();
        }
        if (argResponse.data && argResponse.data.appStatus && argResponse.data.appStatus == "DECLINED"){
...
//      showFaildPandingScreen();
	...
	//---------------------------------------------------------------------------------------
	function showFaildPandingScreen(){
		if (tuFailed){
			showTUFaildPandingScreen();
			return;
		}
	...
	//---------------------------------------------------------------------------------------
	function showTUFaildPandingScreen(){/*First case if response FAILED*/
		app.flowStateHelper.setFlowFinish();
		hideGroupOfBlocksOnResult();
