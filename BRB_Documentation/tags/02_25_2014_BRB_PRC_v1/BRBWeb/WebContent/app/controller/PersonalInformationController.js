...
BRB.PersonalInformationController = function(activationItems, argTranslator, argMessageDialog, argPopup) {
    ...
    var refs = {    		
    		...
    		addressSince_Month				:	'#personalInformation_AddressSince_Month',
    		employmentType_DropDown			:	'#personalInformation_EmploymentType_TextField',
    		employerSince_Month				:	'#personalInformation_EmployerSince_Month',
    		...
    		titlesDropDown					:	'#personalInformation_Title_TextField',
...
/*                {notField:true, name: 'postalcode1',            	value: null, validation: {type: 'postalCodeFirstPart',     message: '', group:[1]}},
                ...
                {name: 'housingpayment',        	value: null, validation: {type: 'format',     message: 'personalInformation_MonthlyHousingPaymentError', group:[1], matcher: /^[0-9\,]{1,4}$/}},
                ...
                {name: 'grossIncome',     			value: null, validation: { type: 'format', message: 'personalInformation_GrossAnnualIncomeError', matcher: /\d+/, group:[5] } },
...
//		bindEmploymentTypeControl();
		...
		if (argPopup) {
        	if (blockName) {
    			if (blockName === refs.blockNameAboutYourself) {
    				$(refs.titlesDropDown).focus();
    				$(refs.titlesDropDown).select();
	...
	function restoreAboutYourselfUserData() {
        ...
        BRB.Log(logPrefix + sMethod);
        $(refs.titlesDropDown).val(app.ie9SelectHelper.getRightSelectValue(model.get('title')));
    ...
    function syncAboutYourselfUserData(currentModel){
        ...
        currentModel.set('title', 			  $(refs.titlesDropDown).val());
        ...
        if (membershipNumber && membershipNumber.length > 0) {        
        	...
        	model.set('loyaltyMembershipNumber', loyaltyMembershipNumberPrefix + model.get('loyaltyMembershipNumberInternal')) ;
        } else {
    ...
    function bindEvents() {
		...
		$(refs.titlesDropDown).change(function() {
			model.set('title', $(refs.titlesDropDown).val());
		});
	...
	function fillControlsWithData() {
		...
		titlesList.fillSelectControl(refs.titlesDropDown);
	...
	function validateAboutYourselfBlock(currentModel) {
        ...
        if (!currentModel.get('title')) {
        	validationResult.push({name: 'title', err: 'personalInformation_TitleError', uiid: refs.titlesDropDown});
        }
        ...
        if ((employmentType != null) && (employmentType != 'null') && (employmentType.toUpperCase() != 'R') && 
        	!currentModel.isEmptyValue('howLongYears') && !currentModel.isEmptyValue('howLongMonthes')) {
       		...
       		if (!currentModel.validateForFutureYearMonth(sinceModelDate, currentModel.get('birthDate'))) {
       			validationResult.push({name: 'howLongYears', err: '', uiid: refs.howLongYears});
       			validationResult.push({name: 'howLongMonthes', err: '', uiid: refs.howLongMonthes});
