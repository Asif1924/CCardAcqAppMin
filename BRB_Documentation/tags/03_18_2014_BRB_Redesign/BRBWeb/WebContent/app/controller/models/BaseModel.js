...
BRB.BaseModel = function(config) {
    ...
    this.getNotEmtyData = function(bIncludeNonFields){
        ...
        $.each(this.data, function(index, item) {
            if(!item.notField || (bIncludeNonFields && item.notField)){
                ...
                if (!_.isEmpty(item.name)  && item.value != null && !_.isEmpty(item.value.toString())) {
                	 rez.push({name: item.name, value: item.value});
                }
            }
        });
    ...
    var isValid = function(validation, name, value){
        ...
        if(isError){
            ...
            BRB.Log('BRB.BaseModel::isValid() name:'+rez.name+'  err:'+rez.err+'  uiid:'+rez.uiid);
        }
        return rez;
    };
    //---------------------------------------------------------------  
    ...
    this.validateForTimePeriod = function(year, month){
    	...
    	if(birthDate.getTime() == date.getTime()) return true;
    ...
    this.getFullYears = function(date)
    {
    	...
    	{
    		var now = moment()._d;
    		fullYears = ( now.getFullYear() - date.getFullYear() - (((now.getMonth() - (date.getMonth()-1))||now.getDate() - date.getDate())<0) );
    	}
    ...
    this.validateAge = function(birthDate, province, birthDateField) {
    	...
    	else if(fullYears == 18 && province !== null && province != '')
    	{
    		if(province === 'NF'  || 
    		    	   ...
    		    	   province === 'NU' )
    		{
    			validationResult= {name: 'birthDate', err: 'personalInformation_DOB_19YearsError', uiid: birthDateField, province: province};
    		}
