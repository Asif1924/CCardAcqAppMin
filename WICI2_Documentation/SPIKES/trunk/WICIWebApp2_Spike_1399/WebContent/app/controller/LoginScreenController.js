...
WICI.LoginScreenController = function() {
	...
	var refs = {
		...
		upcBarcodeContainer : '#UPCBarcode'
	};
	...
	var model = new WICI.BaseModel({
		...
		// {name: 'userID', value: null, validation: {type: 'presence', message:
		// '', group: [1]} },
		// {notField:true, name: 'passwordFieldID', value: null, validation:
		// {type: 'presence', message: '', group: [1]} },
		{
	...
	function init(argTranslator, argMessageDialog) {
		...
		barcodeHelper = new WICI.BarcodeHelper();
	}
	// ---------------------------------------------------------------------------------------
	...
	function createView() {
		...
		// $(refs.agentID).autoNumeric('init', {aSign:'',vMin:'0',
		// vMax:'99999999999999999999', mDec:'0', aSep: ''});
	...
	function bindEvents() {
		...
		$(refs.loginButtonID).click(
				function() {
					// invokeLogin( $(refs.employerID).val(),
					// $(refs.userID).val(), $(refs.passwordFieldID).val(),
					// $(refs.locationFieldID).val(),
					// handleSuccessfulLoginRequest, failedLogin);
					invokeLogin($(refs.employerID).val(),
							$(refs.agentID).val(), "", "", $(
									refs.locationFieldID).val(),
							app.apkVersionHelper.getAPKVersion(),
							handleSuccessfulLoginRequest, failedLogin);
				});
		...
		$(refs.language_choser).on(
				"change",
				function() {
					translator.toggleLanguage(true);
					app.language.setLanguage(translator
							.getCurrentLanguageFSDPFormat());
				});

		// Show device number as UPC barcode
		$(refs.upcBarcodeContainer).on('taphold', showDeviceSNBarcode);
	}
	// ---------------------------------------------------------------------------------------
	function showVersions() {
		var constructedMessage = "WebApp Version = " + WICI.version + "<br>";
		constructedMessage += "APK Version = "
				+ app.apkVersionHelper.getAPKVersion() + "<br>";
		constructedMessage += "Device Info = { MfgSerial : "
				+ app.deviceInfoHelper.getDeviceInfo().MfgSerial
				+ ", BuildSerial : "
				+ app.deviceInfoHelper.getDeviceInfo().BuildSerial + " } <br>";
		constructedMessage += "Physical Server = " + WICI.physicalserverURL
				+ "<br>";
		constructedMessage += "Middleware = " + WICI.middlewareName + "<br>";
		constructedMessage += "Web Application = " + WICI.thisWebappName
				+ "<br>";
	...
	// ---------------------------------------------------------------------------------------
	function showDeviceSNBarcode() {
		var sMethod = 'showDeviceSNBarcode() ';
		console.log(logPrefix + sMethod);
		
		var deviceSerialNumber = app.deviceInfoHelper.getDeviceInfo().MfgSerial; //"RF1C7AY6REM"; 
		var bitmap = barcodeHelper.convertDeviceSerialNumberToBarcode(deviceSerialNumber);		
		
		if (bitmap)
		{			
			messageDialog.barcode(null, "Device SN as Code-128 Barcode", $.noop, null, bitmap);	
		}
	}
	// ---------------------------------------------------------------------------------------
	function invokeLogin(argEmployerID, argAgentID, user, password, locationID,
			apkVersion, argSuccessCB, argFailureCB) {
		...
		/*
		 * var isDemoMode = (user.toLowerCase() ===
		 * WICI.AppConfig.DemoUserCredentials.login && password.toLowerCase()
		 * === WICI.AppConfig.DemoUserCredentials.password && locationID ===
		 * WICI.AppConfig.DemoUserCredentials.locationID);
		 */
		var isDemoMode = (argEmployerID.toLowerCase() === WICI.AppConfig.DemoUserCredentials.employerID
				.toLowerCase()
				&& argAgentID.toLowerCase() === WICI.AppConfig.DemoUserCredentials.agentID
						.toLowerCase() && locationID === WICI.AppConfig.DemoUserCredentials.locationID
				.toLowerCase());

		app.setDemoMode(isDemoMode);

		/*
		 * var isBypassMode = (user.toLowerCase() ===
		 * WICI.AppConfig.AdminBypassCredentials.login && password.toLowerCase()
		 * === WICI.AppConfig.AdminBypassCredentials.password );
		 */
		var isBypassMode = (argEmployerID.toLowerCase() === WICI.AppConfig.AdminBypassCredentials.employerID
				.toLowerCase()
				&& argAgentID.toLowerCase() === WICI.AppConfig.AdminBypassCredentials.agentID
						.toLowerCase() && locationID === WICI.AppConfig.AdminBypassCredentials.locationID
				.toLowerCase());

		if (isBypassMode) {
			// console.log(logPrefix + sMethod + " WARNING! BYPASS CODE IS
			// USED!!!");
		...
		if (apkVersion == null || apkVersion === undefined)
			messageDialog.error(translator
					.translateKey("incorrect_Apk_Version_Dialog"), translator
					.translateKey("errorDialog_defaultTitle"),
					navigator.app.exitApp);
		connectivityController.Login(argEmployerID, argAgentID, user, password,
				locationID, apkVersion, argSuccessCB, argFailureCB);
	}
	// ---------------------------------------------------------------------------------------
	...
	function failedLogin(argResponse) {
		...
		// messageDialog.error(
		// translator.translateKey("loginScreen_FailureMessage"),
		// translator.translateKey("loginScreen_Dialog_ErrorTitle"),
		// enableLoginCredentialsFieldsAndFocusAgentIDField);
		messageDialog.error(translator.translateKey(loginHelper
				.getBundleCodeForErrorMessage()), translator
				.translateKey("loginScreen_Dialog_ErrorTitle"),
				enableLoginCredentialsFieldsAndFocusAgentIDField);
	}
	// ---------------------------------------------------------------------------------------
	function respondToUserLocationLookup(argLookupResponse) {
		...
		if (argLookupResponse) {
			...
			var constructedMessage = translator
					.translateKey("loginScreen_UserLookup_ConfirmMessage");
			constructedMessage += "<br>";
			constructedMessage += "<br>";
			constructedMessage += outletStreet.trim() + "<br>";
			constructedMessage += outletCity.trim() + ", "
					+ outletProvince.trim() + "<br>";
			...
			messageDialog
					.htmlConfirm(
							...
							handleLookupNo,
							translator
									.translateKey("loginScreen_UserLookupDialog_NormalTitle"));
	...
	function handleLookupFailed() {
		...
		messageDialog
				.error(
						translator
								.translateKey("loginScreen_UserLookup_FailedMessage"),
						translator
								.translateKey("loginScreen_UserLookupDialog_ErrorTitle"),
	...
	function handleLookupYes() {
		...
		try {
			...
			app.zebraPrinterWrapper.getStoredPrinterMacAddress(
					retrievePrinterMacAdrSuccess, retrievePrinterMacAdrFailure);
		} catch (err) {
	...
	function showNextScreen() {
		...
		if (outletProvince == 'NS') {
			...
			app.flowController = new WICI.NewCustomerCreditCardAcquisitionFlow(
					creditCardData, translator, messageDialog);
	...
	function checkForPrintSetupWarning() {
		if (!app.accountProfileHelper.isAdminProfile()
				&& !app.zebraPrinterWrapper.verifyPrinterMacAddress()) {
	...
	function showMessageForNS() {
		...
		messageDialog.error(translator.translateKey("infoDialog_NoProduct"),
