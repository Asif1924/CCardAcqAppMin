...
WICI.ChooseProductScreenController = function(activationItems, argTranslator,
	...
	var translator;
	var messageDialog;
	var agencyPromoCode = null;
	var agencyProgram = null;
	...
	function bindEvents() {
		...
		$(refs.agencyProgram).on("change", function(){
			var val = this.value;
			if (val==="other" && $(refs.agencyPromoCode).val()){
				$(refs.agencyPromoCode).val($(refs.agencyPromoCode).val())
									   .removeAttr('disabled');				
			}else if (val==="other" && !$(refs.agencyPromoCode).val()) {
	...
	function showNextScreen() {
		...
		//done for user story 1215, need to replace "BLANK" value
...
//		}
	...
	function populateProgramsList(){
		...
		if (model.get('agencyProgram')){
			$(refs.agencyProgram + " [value='" + model.get('agencyProgram') + "']").attr("selected",
			"selected");
		...
		if(agencyProgram){
			$(refs.agencyProgram + " [value='" +agencyProgram + "']").attr("selected",
			"selected");
			if(agencyPromoCode){
				$(refs.agencyPromoCode).val(agencyPromoCode);	
			}			
			agencyPromoCode = null;
			agencyProgram = null;
		}
  ...
  //---------------------------------------------------------------------------------------
    function restoreProgram() {
    	agencyPromoCode = app.agencyPromoCode;
		agencyProgram = app.agencyProgram;
    };
};
