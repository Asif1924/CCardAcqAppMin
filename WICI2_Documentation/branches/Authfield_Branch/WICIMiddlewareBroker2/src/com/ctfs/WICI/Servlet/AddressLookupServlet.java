...
public class AddressLookupServlet extends WICIServlet
{
	...
	private void invokeAddressLookup(WICIServletMediator requestMediator)
	{
		...
		String streetNumber = requestMediator.isHttpGetRequestParameterExist("streetNumber") ? requestMediator.getHttpGetRequestParameter("streetNumber") : EMPTY_STRING;;
		String postalCode = requestMediator.isHttpGetRequestParameterExist("postalCode") ? requestMediator.getHttpGetRequestParameter("postalCode") : EMPTY_STRING;;
		...
		Holder<MessageHeader> messageHolder = (new WICIObjectsHelper()).createMessageHeader(WICIMessageType.ADDRESS_LOOKUP);
		...
		try
		{
			requestBody.setRequestBody(new AddressLookupHelper().addressLookupSerialize(addressLookupRequest));
	...
	private WICIResponse formatOutputForTablet(ResponseBody responseBody)
	{
		...
		if (rawResponseBody != null)
		{
			...
			if (resultDocument != null)
			{
				...
				StringBuffer WithoutFrenchSpecialChars = new WICIServletMediator().replaceFrenchCharsForEquivalent(new StringBuffer(resultDocument));
