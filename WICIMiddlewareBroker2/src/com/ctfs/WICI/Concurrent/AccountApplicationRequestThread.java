...
public class AccountApplicationRequestThread implements Work
{
	...
	public void run()
	{
		...
		if( "PENDING".equalsIgnoreCase(submissionResponse.getAppStatus() ) )
		 ...
		 /* if( "APPROVED".equalsIgnoreCase(submissionResponse.getAppStatus()) && !wicidbHelper.isEmailSendForApprovedApp(transactionID)){
				
				log.info(sMethod + "::send email  addproved case : \n" + submissionResponse.getAppStatus() + "transactionID   ::" +transactionID);
			
				EmailServiceHelper emailServiceHelper = new EmailServiceHelper();
				
				emailServiceHelper.sendEmail(transactionID,submissionResponse);
