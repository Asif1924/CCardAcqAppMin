...
public class CheckCPEligibilityServlet2 extends WICIServlet {
	private static final long serialVersionUID = 1L;
	
	static Logger log = Logger.getLogger(CheckCPEligibilityServlet2.class.getName());

	public CheckCPEligibilityServlet2() { }

	protected void handleRequest(WICIServletMediator requestMediator) throws ServletException, IOException {
		log.info("CheckCPEligibilityServlet2[doPost]");
        checkCPEligibility(requestMediator);
	}

	private void checkCPEligibility(WICIServletMediator requestMediator) {
		log.info("CheckCPEligibilityServlet2[checkCPEligibility]Called !");
	...
	private WICIDSSCheckCPEligibilityResponse checkCPEligibilityHttpsClientCall(WICIDSSCheckCPEligibilityRequest checkCPEligibilityDSSRequest){
		...
		WICIDSSCheckCPEligibilityResponse checkCPEligibilityResponse = new WICIDSSCheckCPEligibilityResponse();
		log.info("CheckCPEligibilityServlet2[checkCPEligibilityHttpsClientCall]");
		try{
			HttpClientHelper secureClient = new HttpClientHelper();
			httpClient = secureClient.getHttpSecureClient();
			WICIConfiguration conf = new WICIConfigurationFactory().createReWriteDASSEndPointConfiguration();
			HttpPost post = new HttpPost(conf.getRewritecpEligibilityEndPoint());
			...
			log.info("CheckCPEligibilityServlet2[checkCPEligibilityHttpsClientCall] The DSS CheckCPEligibility endPoint  === " + CWE117Fix.encodeCRLF(conf.getRewritecpEligibilityEndPoint()) + " CheckCPEligibility request for dss ::: "+CWE117Fix.encodeCRLF(jsonInput));
			HttpResponse response = httpClient.execute(post);
			int statusCode = response.getStatusLine().getStatusCode();
			log.info("CheckCPEligibilityServlet2[checkCPEligibilityHttpsClientCall] CheckCPEligibility Profile status code :::: " + CWE117Fix.encodeCRLF(String.valueOf(statusCode)));
			if (statusCode != 200) {
			...
			log.info("CheckCPEligibilityServlet2[checkCPEligibilityHttpsClientCall] CheckCPEligibility Profile Response from DSS:::: " + CWE117Fix.encodeCRLF(checkCPEligibilityResponse != null ?checkCPEligibilityResponse.toString() : null));
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	private WICIDSSCheckCPEligibilityResponse checkCPEligibilityHttpClientCall(WICIDSSCheckCPEligibilityRequest checkCPEligibilityDSSRequest) {
		...
		try{
	    	...
	    	log.info("checkCPEligibilityHttpClientCall[]:: The DSS CheckCPEligibility endPoint  === " + conf.getCpEligibilityEndPoint() + " CheckCPEligibility request for dss ::: "+jsonInput);
	    	post.setEntity(new StringEntity(jsonInput));
	    	post.setHeader("Content-type", "application/json");
	    	HttpResponse response = httpClient.execute(post);
	        ...
	        if( checkCPEligibilityResponse != null ){
	        	log.info("checkCPEligibilityHttpClientCall[]:: checkCPEligibilityResponse Response "+ CWE117Fix.encodeCRLF(checkCPEligibilityResponse != null ?checkCPEligibilityResponse.toString() : null));
	        }
	    } catch(Exception e){
